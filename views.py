...
def vl_query(request):
	...
	if request.method == 'POST':
		...
		records = requests.post ('http://127.0.0.1:9200/test7/mappedTrials/_search?size=10', data=json.dumps (body))
		total = records.json ()["hits"]["total"]
		Q = Query(disease=disease,age=age,gender=gender,gene=gene,aas=aas,stage=stage,grade=grade,resultNUM=total)
		Q.save()


	else:
		form = QueryForm ()
	return render (request,'query.html', {'obj': Query.objects.all (),'form':form})
